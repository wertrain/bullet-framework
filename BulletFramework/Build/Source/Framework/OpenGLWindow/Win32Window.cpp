...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_KEYUP:
		{
			int keycode = getSpecialKeyFromVirtualKeycode(static_cast<int>(wParam));
			switch (keycode)
			{
				case B3G_ALT:
				{
		...
		case WM_MBUTTONUP:
		{
			...
			if (sData)
			{
				...
				if (sData && sData->m_mouseButtonCallback)
					(*sData->m_mouseButtonCallback)(1, 0, static_cast<float>(xPos), static_cast<float>(yPos));
			}
			break;
		}
		case WM_MBUTTONDOWN:
		{
			...
			if (sData)
			{
				...
				sData->m_mouseYpos = yPos;
				if (sData && sData->m_mouseButtonCallback)
					(*sData->m_mouseButtonCallback)(1, 1, static_cast<float>(xPos), static_cast<float>(yPos));
			}
			break;
		}
		...
		case WM_LBUTTONUP:
		{
			...
			if (sData)
			{
				...
				if (sData && sData->m_mouseButtonCallback)
					(*sData->m_mouseButtonCallback)(0, 0, static_cast<float>(xPos), static_cast<float>(yPos));
			}
		...
		case WM_LBUTTONDOWN:
		{
			...
			if (sData)
			{
				...
				if (sData && sData->m_mouseButtonCallback)
					(*sData->m_mouseButtonCallback)(0, 1, static_cast<float>(xPos), static_cast<float>(yPos));
			}
			break;
		}
		...
		case WM_MOUSEMOVE:
		{
			...
			if (sData && sData->m_mouseMoveCallback)
				(*sData->m_mouseMoveCallback)(static_cast<float>(xPos), static_cast<float>(yPos));
		...
		case WM_RBUTTONUP:
		{
			...
			if (sData && sData->m_mouseButtonCallback)
				(*sData->m_mouseButtonCallback)(2, 0, static_cast<float>(sData->m_mouseXpos), static_cast<float>(sData->m_mouseYpos));
		...
		case WM_RBUTTONDOWN:
		{
			...
			if (sData && sData->m_mouseButtonCallback)
				(*sData->m_mouseButtonCallback)(2, 1, static_cast<float>(sData->m_mouseXpos), static_cast<float>(sData->m_mouseYpos));
			...
			switch (wParam)  // Evaluate Size Action
			{
				...
				case SIZE_RESTORED:   // Was Window Restored?
					...
					if (sData->m_resizeCallback)
					{
						...
						(*sData->m_resizeCallback)(static_cast<float>(sData->m_openglViewportWidth), static_cast<float>(sData->m_openglViewportHeight));
					}
...
void Win32Window::setResizeCallback(b3ResizeCallback resizeCallback)
{
	...
	if (m_data->m_resizeCallback)
		(*m_data->m_resizeCallback)(static_cast<float>(m_data->m_openglViewportWidth), static_cast<float>(m_data->m_openglViewportHeight));
}
